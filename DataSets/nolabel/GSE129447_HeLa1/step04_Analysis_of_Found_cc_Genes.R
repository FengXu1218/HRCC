...
for (i in 5:12) {
  p_list[[n]] <- pca_scatter_plot(prcomp(t(exp_cpm[intersect(rownames(exp_cpm),
                                                             get(paste0("gene_", i))),])),
                                  add_legend = F,
                                  title = "HeLa1", sub_title = paste0(length(get(paste0("gene_", i))),
...
for (i in 1:nrow(data_cc_bind)) {
  res <- fftf(1:ncol(data_cc_bind), exp(as.numeric(data_cc_bind[i,])), 1/(ncol(data_cc_bind)/6), plot = T)
  X_fftf[i,] <- res[[1]]
  print(i)
}

scale()
# write.csv(X_fftf, paste0(dir_path, "X_FFTF_data/HeLa1_X_fftf2_152.csv"))
write.table(round(t(X_fftf[,(ncol(X_fftf)/3+1):(ncol(X_fftf)*2/3)]),3),
            paste0(dir_path, "X_FFTF_data/HeLa1_X_fftf_152.txt"), sep = ",",
...
p_ind <- heatmap_plot(X_fftf, name = "HeLa1_152_cc_genes", output = T, save_path = save_path)

pca_scatter_plot(prcomp(t(X_fftf[,(ncol(X_fftf)/3+1):(ncol(X_fftf)*2/3)])), title = "X_fftf_HeLa1")

...
HeLaKO <- read.table(paste0(dir_path, "rawdata/GSM4224316_out_gene_exon_tagged.dge_exonssf002_KO.txt"),
                    header = T, row.names = 1)

# Check the sequencing depth distribution:
quantile(colSums(HeLaKO), probs = c(0,0.1, 0.25,0.5,0.75,0.9, 1))

# Cells with sequencing depth less than 10000 were removed:
colids <- which(colSums(HeLaKO) > 10000)

# Genes with 0 expression in more than 70% of all cells were removed:
rowids <- which(apply(HeLaKO, 1, function(x) length(which(x == 0))/length(x) <= 0.92))

exp <- HeLaKO[rowids, colids]

# cpm transformation：
exp_cpm <- cpm(exp, log = T) %>% round(2)
exp_cpm_nolog <- cpm(exp, log = F) %>% round(2)

# write.csv(exp_cpm, paste0(dir_path, "cpm/HeLaKO_cpm.csv"))
# write.csv(exp_cpm_nolog, paste0(dir_path, "cpm/HeLaKO_cpm_nolog.csv"))
...
# write.csv(data_cc, paste0(dir_path, "cpm/HeLaKO_cc_cpm.csv"))
...
# write.csv(data_cc_new, paste0(dir_path, "filtered_data/HeLaKO_cc_cpm_filter.csv"))

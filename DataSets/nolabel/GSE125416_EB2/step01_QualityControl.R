...
EB2 <- Read10X(paste0(dir_path, "rawdata/"))
EB2 <- as.data.frame(EB2)

# Check the sequencing depth distribution:
quantile(colSums(EB2), probs = c(0,0.1, 0.25,0.5,0.75,0.9, 1))

# Cells with sequencing depth less than 10000 were removed:
colids <- which(colSums(EB2) > 10000)

# Genes with 0 expression in more than 70% of all cells were removed:
rowids <- which(apply(EB2, 1, function(x) length(which(x == 0))/length(x) <= 0.93))

exp <- EB2[rowids, colids]

# cpm transformation：
exp_cpm <- edgeR::cpm(exp, log = T) %>% round(2)
exp_cpm_nolog <- cpm(exp, log = F) %>% round(2)

# write.csv(exp_cpm, paste0(dir_path, "cpm/EB2_cpm.csv"))
# write.csv(exp_cpm_nolog, paste0(dir_path, "cpm/EB2_cpm_nolog.csv"))
...
# write.csv(data_cc, paste0(dir_path, "cpm/EB2_cc_cpm.csv"))
...
write.csv(data_cc_new, paste0(dir_path, "filtered_data/EB2_cc_cpm_filter.csv"))

...
#                                                   seurat_cc_genes),])),
#                        add_legend = F,
#                        title = "EB1", sub_title = "97 cell cycle-related genes from Seurat")
# #p1
...
#                                                              get(paste0("gene_", i))),])),
#                                   add_legend = F,
#                                   title = "EB1", sub_title = paste0(length(get(paste0("gene_", i))),
...
X_fftf_names <- c("HeLa1", "EB1")
inter_genes <- intersect(rownames(X_fftf_HeLa1), rownames(X_fftf_EB1))
...
# X_fftf_EB1 <- X_fftf_EB1[,ncol(X_fftf_EB1):1]
...
# write.csv(X_fftf, paste0(dir_path, "X_FFTF_data/EB1_X_fftf2_152.csv"))
write.table(round(t(X_fftf[,(ncol(X_fftf)/3+1):(ncol(X_fftf)*2/3)]),3),
            paste0(dir_path, "X_FFTF_data/EB1_X_fftf_152.txt"), sep = ",",
...
p_ind <- heatmap_plot(X_fftf, name = "EB1_152_cc_genes", output = T, save_path = save_path)

pca_scatter_plot(prcomp(t(X_fftf[,(ncol(X_fftf)/3+1):(ncol(X_fftf)*2/3)])), title = "X_fftf_EB1")

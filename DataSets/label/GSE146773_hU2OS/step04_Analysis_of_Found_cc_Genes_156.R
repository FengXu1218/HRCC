...
# pca scatter plot:
# p1 <- pca_scatter_plot(prcomp(t(exp_tpm[intersect(rownames(exp_tpm),
#                                                   seurat_cc_genes),])),
#                        add_legend = F,
#                        title = "hU2OS", sub_title = "97 cell cycle-related genes from Seurat")
# #p1
# p2 <- pca_scatter_plot(prcomp(t(exp_tpm[intersect(rownames(exp_tpm),
#                                                   caff_cc_genes),])),
#                        add_legend = F,
#                        title = "hU2OS",
...
# n = 3
# for (i in 5:14) {
#   p_list[[n]] <- pca_scatter_plot(prcomp(t(exp_tpm[intersect(rownames(exp_tpm),
#                                                              get(paste0("gene_", i))),])),
#                                   add_legend = F,
#                                   title = "hU2OS", sub_title = paste0(length(get(paste0("gene_", i))),
...
cc_genes <- intersect(gene_8, rownames(exp_tpm))
...
X_fftf_names <- c("HeLa1", "hU2OS")
inter_genes <- intersect(rownames(X_fftf_HeLa1), rownames(X_fftf_hU2OS))
...
# X_fftf_hU2OS <- X_fftf_hU2OS[,ncol(X_fftf_hU2OS):1]
#
# for (i in 1:length(X_fftf_names)) {
#   assign(paste0("rank_", X_fftf_names[i]),
#          rank(get_row_order(get(paste0("X_fftf_", X_fftf_names[i]))[inter_genes,])[[1]]))
...
# write.csv(X_fftf, paste0(dir_path, "X_FFTF_data/hU2OS_X_fftf2_152.csv"))
write.table(round(t(X_fftf[,(ncol(X_fftf)/3+1):(ncol(X_fftf)*2/3)]),3),
            paste0(dir_path, "X_FFTF_data/hU2OS_X_fftf_152.txt"), sep = ",",
...
p_ind <- heatmap_plot(X_fftf, name = "hU2OS_152_cc_genes", output = T, save_path = save_path)

pca_scatter_plot(prcomp(t(X_fftf[,(ncol(X_fftf)/3+1):(ncol(X_fftf)*2/3)])), title = "X_fftf_hU2OS")
